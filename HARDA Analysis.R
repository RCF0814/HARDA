...
library(drc)
library(progress)
data <- read.csv(file="RCF20201209_Staurosporine_SIP8_Report.csv",header=TRUE)
## experiment design
...
table(data_Final[data_Final$is.Target=="yes",]$is.Kinase)

# output data
write.csv(file=paste0("data_mean_",temp,"_final.csv"),data_Final,row.names = FALSE)

# plotting
data_Final <- data_Final %>% arrange(is.Kinase.KinHub)

p <- ggplot(data=data_Final)+
  geom_point(aes(x=10^Correlations,y=Log10P.Value_20uM_0nM,col=is.Kinase.KinHub),size=0.3,alpha=0.6)+
  scale_x_continuous(name = expression("Correlations "*"("*R^2*")"),limits = c(0,10),breaks = c(0,3.16228,10),labels = c(0,0.5,1))+
  scale_y_continuous(expression("-"*Log[10 ]*p*"-"*value),limits = c(0,12),breaks=c(0,4,8,12),labels = c(0,4,8,12))+
  scale_color_manual("",breaks = c("no","yes","no(FD)"),values = c("lightsteelblue3","red","gray45"),labels=c("Others","Kinase",""))+
  stat_function(fun=cur.f,xlim = c(6.31,10),size=0.15)+
  geom_vline(xintercept = 6.31,linetype="dashed",size=0.15)+
  geom_hline(yintercept = 2.3,linetype="dashed",size=0.15)+
  theme_light()+
  annotate(geom="text",x=0.95,y=2.8,label="paste(italic(P),\" = 0.005\")",parse=TRUE,size=1.75)+
  annotate(geom="text",x=5.8,y=11,label="paste(italic(R) ^ 2, \" = 0.8\")",parse = TRUE,angle=90,size=1.75)+
  theme(legend.position = c(0.15,0.91),legend.spacing.x = unit(0.01,"cm"),legend.margin = margin(-11,0,0,0),legend.background = element_blank(),
        legend.key.size = unit(5,"points"),legend.text=element_text(size = 5),legend.spacing.y = unit(0,"cm"),
        axis.title.x = element_text(size=6,margin = margin(-0.5,0,-7,0)),
        axis.title.y = element_text(size=6,margin = margin(0,-2,0,-5.5)),
        axis.text.x = element_text(size=5,colour = "black",margin = margin(0.8,0,0,0)),
...
p2 <- ggMarginal(p,type="density",groupColour = T,size = 5, xparams = list( size = 0.25),yparams = list( size = 0.25))


ggsave("SC_heat.pdf",p2,width = 4.6,height = 4.5,units = "cm")
